...
import { IGenericResponse } from '../interfaces/response.interface'
import { SituacaoEnum } from '../interfaces/situacao.interface'
import { ITitular, ITitularEdicaoDto } from '../interfaces/titular.interface'
import { cliente } from '../server/axios.client'

export function EdicaoTitular(){

    const {state} = useLocation()

    useEffect(()=> {
        cliente.get(`titular/${state.id}`).then(({data}:IGenericResponse<ITitular>)=>{
            setNomeTitularEdit(data.nome)
    ...
    const [nomeTitularEdit, setNomeTitularEdit] = useState('')

    function onSubmitTitularEdit(){
        const titularEdit: ITitularEdicaoDto = {
            nome: nomeTitularEdit,
            situacao,
            modalidade
        }
        cliente.put(`titular/${state.id}`, titularEdit)
    }
    ...
    return(
        ...
        >
            ...
            <Text bg={'whiteAlpha.900'} textAlign={'center'} padding={2} fontWeight={'bold'} borderRadius={4}>Editar dados titular</Text>
            ...
            <Grid gridTemplateColumns={'0.2fr 1fr'} rowGap={2}>
                ...
                <GridItem>
                        <Input bg={'whiteAlpha.800'} value={nomeTitularEdit} onChange={(event)=>{setNomeTitularEdit(event.target.value)}}/>
                </GridItem>
            ...
            <Flex justifyContent={'flex-end'}>
                <Flex gap={'3'}>
                    <Button colorScheme={'green'}  onClick={()=>{onSubmitTitularEdit()}}>Salvar</Button>
                </Flex>
            </Flex>
